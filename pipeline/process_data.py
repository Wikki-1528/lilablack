...
Output:
  ...
  ../public/data/analytics/{mapId}.json
...
def main():
    ...
    for match_id, match in all_matches.items():
        ...
        kills        = sum(1 for p in players_list for e in p["events"] if e["event"] in ("Kill", "BotKill"))
        bot_kills    = sum(1 for p in players_list for e in p["events"] if e["event"] == "BotKill")
        deaths       = sum(1 for p in players_list for e in p["events"] if e["event"] in ("Killed", "BotKilled", "KilledByStorm"))
